/**
 * https://www.codewars.com/kata/50654ddff44f800200000007/train/javascript
 * Given 2 strings, a and b, return a string of the form short+long+short, with the shorter string on the outside and the longer string
 *  on the inside. The strings will not be the same length, but they may be empty ( zero length ).

Hint for R users:

The length of string is not always the same as the number of characters
For example: (Input1, Input2) --> output

("1", "22") --> "1221"
("22", "1") --> "1221"
 */


// Entry data
const long1 = "Soon";
const long2 = "22";

const short1 = "Me";
const short2 = "1";



// Solution 1
const solution = (a, b) => a.length < b.length ? (a + b + a) : (b + a + b);


// Solution 2
const solution2 = (a, b) => {
    let resultStr = "";
    if (a.length < b.length) {
        resultStr += a + b + a;
    } else {
        resultStr += b + a + b;
    }

    return resultStr;
};


// Examples
console.log(solution(long1, short1));
console.log(solution(long2, short2));

console.log(solution2(long1, short1));
console.log(solution2(long2, short2));
